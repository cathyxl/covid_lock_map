"""
Change weibo search into automatically search by short interval in case of emission.
Only 50 page count may not be enough.
"""
import re
import scrapy
...
class SearchDaySpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        while cur_start_date < end_date:
            ...
            start_str = cur_start_date.strftime('%Y-%m-%d') + '-0'
            end_str = (cur_start_date + timedelta(days=3)).strftime('%Y-%m-%d') + '-0' \
                if cur_start_date + timedelta(days=3) < end_date \
                else end_date.strftime('%Y-%m-%d') + '-0'

            for keyword in self.keyword_list:
                if not self.settings.get('REGION') or '全部' in self.settings.get(
                        'REGION'):
                    print("search %s - %s, keyword %s" % (start_str, end_str,keyword))
